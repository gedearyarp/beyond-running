...
export default function ProductDetailPage({ product, relatedProducts }: ProductDetailPageProps) {
    ...
    // Ekstrak Colors dan Sizes dari varian produk
    const availableColors = Array.from(new Set(
        product.variants.edges.flatMap(v =>
    ...
    const [selectedColor, setSelectedColor] = useState<string | null>(availableColors.length > 0 ? availableColors[0] : null);
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        ...
                        <div className="w-full flex md:justify-end">
                            <div className="md:w-2/3 md:px-0 px-4">
                                ...
                                <h1 className="text-3xl font-bold font-avant-garde mb-1">{product.title}</h1>
                                {/* Anda bisa tambahkan kategori atau gender di sini dari product.productType atau tags */}
                                <h2 className="text-2xl font-bold font-avant-garde mb-6">{/* PRODUCT GENDER or Category */}</h2>
                                <p className="text-xl font-avant-garde mb-8">{formattedPrice}</p>

                                <p className="text-sm font-avant-garde mb-10" dangerouslySetInnerHTML={{ __html: product.descriptionHtml }}></p>
                                ...
                                <div className="border-t border-gray-200 py-4">
                                    ...
                                    {expandedSections.technical && (
                                        <ul className="mt-4 pl-5 text-sm font-avant-garde space-y-3">
                                            {/* Ini adalah dummy content, ganti dengan data dari product.descriptionHtml atau metafields */}
                                    ...
                                    <div className="flex space-x-3">
                                        {availableColors.length > 0 ? (
                                            availableColors.map((color) => (
                                                <button
                                                    key={color}
                                                    // Anda mungkin perlu mapping nama warna ke kode hex jika tidak disimpan di Shopify
                                                    className={`w-10 h-10 rounded-full ${selectedColor === color ? "ring-2 ring-offset-4 ring-black" : "border border-gray-300"}`}
                                                    style={{ backgroundColor: color.toLowerCase() === 'black' ? '#000000' : color.toLowerCase() === 'white' ? '#FFFFFF' : '#cccccc' }} // Contoh sederhana mapping warna
