...
import { LocalizationProvider, LocalizationData } from "@/contexts/LocalizationContext";
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
    // Get localization data from headers set by middleware
    const headersList = await headers();
    const countryCode = headersList.get("x-country-code") || "ID";
    const currencyCode = headersList.get("x-currency-code") || "IDR";
    const locale = headersList.get("x-locale") || "id-ID";
    ...
    return (
        <html lang={locale.split("-")[0]}>
            <body className={`${inter.className} font-avant-garde`}>
                <LocalizationProvider data={localizationData}>
                    <AuthProvider>
                        <LoadingProvider>
