        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            {peripherals.map((peripheral) => {
                ...
                const formattedDate = peripheral.event_date
                    ? new Date(peripheral.event_date)
                ...
                const imageUrl =
                    peripheral.main_img && peripheral.main_img.trim() !== ""
                        ? peripheral.main_img
                ...
                return (
                    <Link
                        key={peripheral.id}
                        href={`/peripherals/${peripheral.id}`}
                        className="group"
                    >
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-white z-20 p-6 text-center">
                                <div className="flex flex-col gap-8 items-center text-center">
                                    <div className="absolute top-2 md:top-8">
                                        <p className="text-[6.5px] md:text-[12px] font-itc-xl">
                                            {formattedDate}
                                        </p>
                                    </div>
                                    <RichTextViewer
