...
export default function CartDropdown() {
    ...
    const handleOutOfStockModalClose = async () => {
        ...
        const remainingItems = items.filter(
            (item) => !outOfStockItems.some((outOfStockItem) => outOfStockItem.id === item.id)
        );
    ...
    return (
        <>
            ...
            >
                {/* Header */}
                <div
                    className={`flex items-center justify-between p-4 md:p-6 border-b border-gray-200 flex-shrink-0 transition-all duration-300 ${
                        isVisible && !isClosing
                    ...
                    {items.length === 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full px-4 md:px-6">
                            <div className="text-center mb-8 font-folio-bold">
                                <h3 className="text-xl md:text-2xl text-gray-900 mb-6 md:mb-8">
                                    Your Cart is Empty
                                </h3>
                                <Link
                                    href="/shop"
                            ...
                            {items.map((item, index) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0 mt-4">
                                        <h3 className="font-itc-demi text-sm md:text-lg mb-1 md:mb-2 truncate">
                                            {item.title}
                                        </h3>
                                        ...
                                        <div className="flex items-center gap-3 md:gap-4 mb-2 md:mb-4">
                                            ...
                                            <span className="w-6 md:w-8 text-center text-sm md:text-sm font-itc-demi">
                                                {item.quantity}
                                            </span>
                                            <button
                                                onClick={() =>
                ...
                {items.length > 0 && (
                    <div
                        className={`border-t border-gray-200 p-4 md:p-6 flex-shrink-0 transition-all duration-300 ${
                            isVisible && !isClosing
                                ? "opacity-100 translate-y-0"
                                : "opacity-0 translate-y-4"
                        }`}
              ...
              <button className="text-xs md:text-sm underline hover:no-underline transition-all text-left md:text-right">
                Apply at checkout
                        ...
                        {/* Tax Notice */}
                        <p className="text-xs font-folio-bold mt-2 md:mt-3">
